...
export function renderFacilitiesPage() {
  ...
  const allList = state.facilities || [];
  ...
  <section class="content">
    <div class="section-head">
      <div>
        ...
        <h2>${t('facilities.title') || 'Corridor Accessibility Facilities'}</h2>
    ...
    ${noRoute
      ...
      ? `<div class="empty"><div><div style="font-size:32px;color:var(--teal)">⟳</div><b>${t('facilities.loading') || 'Discovering all accessibility facilities along corridor…'}</b></div></div>`
      ...
      : `
        <div style="display:flex;flex-wrap:wrap;gap:10px;margin-bottom:18px;align-items:center;">
          <select style="background:var(--card);color:var(--text);border:1px solid var(--border);border-radius:8px;padding:8px 14px;font-size:13px;"
            ...
            <option value="atm" ${filterCat === 'atm' ? 'selected' : ''}>🏧 ATMs & Banking (${countFor('atm')})</option>
          ...
          <div style="color:var(--muted);font-size:13px;margin-left:auto;">
            ...
            <span class="badge" style="margin-left:10px">${displayFacilities.length} places shown</span>
          </div>
        </div>
...
window.selectFacility = async (id) => {
  ...
  notify(`Directing to ${facility.name} on map…`, 'info');
  go('Live Network');
};
